const queries = {
	select: {
		...
		getAllContactEmails: "SELECT id, email_type, email_direction FROM contact_email WHERE user_id = $1",
		...
		getAllContactURLs: "SELECT id, url, type FROM contact_url WHERE user_id = $1",
		...
		getAllContactDates: "SELECT id, date_type, contact_date FROM contact_date WHERE user_id = $1",
		...
		getContactsByGroup:
			"SELECT c.id, c.first_name, c.last_name, c.contact_alias, c.color, cp.phone_code, cp.phone_number FROM contact c LEFT JOIN contact_phone cp ON c.id = cp.contact_id AND cp.user_id = c.user_id AND cp.id = 1 WHERE c.user_id = $1 AND c.id IN (SELECT contact_id FROM contact_group WHERE user_id = $1 AND group_id = $2);",
	},
	insert: {
